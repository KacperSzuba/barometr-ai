...
class ProvenanceSpan(BaseModel):
    ...
    def validate_against_text(self, source_text: str) -> bool:
        ...
        if self.char_start < 0 or self.char_end > len(source_text) or self.char_start >= self.char_end:
            return False
        return not (bool(self.exact_quote) and source_text[self.char_start:self.char_end] != self.exact_quote)
